...
void SCU_SetUsartMode(USART_TypeDef *Usartx, uint32_t mode)
{
  ...
  if (Usartx == UART0)
  {
    SCU->EX_PERIPH_DRIVER_USART0_MODE = ((SCU->EX_PERIPH_DRIVER_USART0_MODE) & (~((0x03UL << 30) | (0x07UL << 21)))) | (invert38k << 31) | (uartmod << 30) | (uartdcsr << 21);
    SCU->EX_USART_MODE                = (SCU->EX_USART_MODE & (~(1 << 12))) | (opendrain << 12);
  }
  else if (Usartx == UART1)
  {
    SCU->EX_PERIPH_DRIVER_USART0_MODE = ((SCU->EX_PERIPH_DRIVER_USART0_MODE) & (~(0x07UL << 24))) | (uartdcsr << 24);

    SCU->EX_USART_MODE = ((SCU->EX_USART_MODE) & (~(0x03UL << 6))) | (invert38k << 7) | (uartmod << 6);
    SCU->EX_USART_MODE = (SCU->EX_USART_MODE & (~(1 << 13))) | (opendrain << 13);
  }
  else if (Usartx == UART2)
  {
    SCU->EX_PERIPH_DRIVER_USART0_MODE = ((SCU->EX_PERIPH_DRIVER_USART0_MODE) & (~(0x07UL << 27))) | (uartdcsr << 27);

    SCU->EX_USART_MODE = ((SCU->EX_USART_MODE) & (~(0x03UL << 8))) | (invert38k << 9) | (uartmod << 8);
    SCU->EX_USART_MODE = (SCU->EX_USART_MODE & (~(1 << 14))) | (opendrain << 14);
  }
  else   // if(Usartx == ((uint32_t)(UART0)))
  {
    SCU->EX_USART_MODE = ((SCU->EX_USART_MODE) & (~(0x1FUL << 1))) | (invert38k << 5) | (uartmod << 4) | (uartdcsr << 1);
...
void SCU_DisableALLGPIOInput(void)
{
#if ZB202
