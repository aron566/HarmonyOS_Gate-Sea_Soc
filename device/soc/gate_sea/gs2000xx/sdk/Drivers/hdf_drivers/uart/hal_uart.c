...
void USART_DMA_IRQHandler(uint8_t num)
{
  ...
  if (RESET != usart_interrupt_flag_get(g_UartPeriph[num], USART_INT_FLAG_IDLE))
  {
    ...
    HDF_LOGI("RECV %d, dma get num %d, date:%s\r\n", USART_RX_NUM, dma_transfer_number_get(g_UartDmaFuncMaps[uartDma->rx_DMA], g_UartDmaChMaps[uartDma->rx_DmaCh]), rx_buffer);
...
void usart_dma_rx_config(uint32_t com, dma_single_data_parameter_struct *dma_rx_init_struct, uartDmaCfg *uartDma)
{
  ...
  dma_channel_subperipheral_select(g_UartDmaFuncMaps[uartDma->rx_DMA],
                                   g_UartDmaChMaps[uartDma->rx_DmaCh],
                                   uartDma->rx_DmaPerSel);

  dma_channel_enable(g_UartDmaFuncMaps[uartDma->rx_DMA], g_UartDmaChMaps[uartDma->rx_DmaCh]);
}

static void usart_dma_tx_config(uint32_t com, dma_single_data_parameter_struct *dma_tx_init_struct, uint8_t *data, uint32_t number, uartDmaCfg *uartDma)
{
...
void com_init_cfg(uint32_t com, UartResource *resource)
{
  ...
  gpio_af_set(g_gpioxMaps[resource->uartPin.txPort], g_gpioAFMap[resource->uartPin.tx_AF], g_GdRealPinMaps[resource->uartPin.txPin]);

  /* connect port to USARTx_Rx */
  gpio_af_set(g_gpioxMaps[resource->uartPin.rxPort], g_gpioAFMap[resource->uartPin.rx_AF], g_GdRealPinMaps[resource->uartPin.rxPin]);

  /* configure USART Tx as alternate function push-pull */
  gpio_mode_set(g_gpioxMaps[resource->uartPin.txPort], GPIO_MODE_AF, GPIO_PUPD_PULLUP, g_GdRealPinMaps[resource->uartPin.txPin]);
  gpio_output_options_set(g_gpioxMaps[resource->uartPin.txPort], GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, g_GdRealPinMaps[resource->uartPin.txPin]);

  /* configure USART Rx as alternate function push-pull */
  gpio_mode_set(g_gpioxMaps[resource->uartPin.rxPort], GPIO_MODE_AF, GPIO_PUPD_PULLUP, g_GdRealPinMaps[resource->uartPin.rxPin]);
  gpio_output_options_set(g_gpioxMaps[resource->uartPin.rxPort], GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, g_GdRealPinMaps[resource->uartPin.rxPin]);
                          ...
                          UartResource                     *resource)
{
  ...
  HDF_LOGI("%s: rxPort %u, rxPin %u, txPort %u, txPin %u, rx_AF %u, tx_AF %u, \
           ...
           uartConfig->rx_AF,
           uartConfig->tx_AF,
           uartDmaConfig->rx_DMA,
           uartDmaConfig->tx_DMA,
           uartDmaConfig->rx_DmaCh,
           uartDmaConfig->tx_DmaCh);
