 ...
 *  @brief 串行外设操作接口.
 *
 *  @details v1.0.0 aron566 2022.10.18 10:20 首个版本，使用空闲中断加FIFO加乒乓双缓冲优化.
             ...
             v1.0.1 aron566 2022.12.7 10:43 增加串行CAN接口.
...
#define USE_DMA_CIRCULAR_MODE     0 /**< 已使用硬件DMA循环接收模式，1代表打开，0代表未打开（未打开或者主动设置为0时，接收到数据将先停止接收再主动重启接收） */
#define USE_LOOPBACK              0 /**< 是否使用数据回环打印 */
#define SERIAL_PORT_DEBUG_UART    &Driver_USART3 /**< 使用Printf接口的串口 */
...
/* 串口缓存 */
static CQ_handleTypeDef Serial_Port_Uart3_CQ;
static uint8_t UART3_CQ_Buffer[512U] = {0};
static uint8_t UART3_DMA_Buf[256U] = {0};
 ...
 * @param event 事件
 */
static void SERIAL3_Event_Process(uint32_t event)
{
  if(ARM_USART_EVENT_HALF_RX_COMPLETE & event)
  {
    /* 半接收中断 */
    Serial_Port_Half_IT_CallBack((void *)&Driver_USART3);
  }

  if (ARM_USART_EVENT_RECEIVE_COMPLETE & event)
  {
    /* 接收完成 or 接收空闲 */
    Serial_Port_Complete_IT_CallBack((void *)&Driver_USART3);
  }
 ...
 * @brief 串口发送接口
 *
 * @param pUartHandle 串口句柄
 * @param Data 数据
 * @param Len 数据长度
 * @param Time 阻塞时间
 * @return HAL_StatusTypeDef
 */
static HAL_StatusTypeDef HAL_UART_Transmit(const void *pUartHandle, const uint8_t *Data, uint32_t Len, uint32_t Time)
{
  HAL_StatusTypeDef ret = HAL_OK;
  ARM_DRIVER_USART *phuart = (ARM_DRIVER_USART *)pUartHandle;
  if(0U < Time)
  {
    /* 等待发送完成 */
    phuart->Send(Data, Len);
    while(0 != phuart->GetStatus().tx_busy)
    {
      /* todo */
    }
  }
...
static void UART_Init(void *pSerial_Port_Handle)
{
  ...
  ARM_DRIVER_USART *phuart = (ARM_DRIVER_USART *)pSerial_Device->pSerial_Handle;
  phuart->Control(ARM_USART_ABORT_SEND, 0);
  phuart->Control(ARM_USART_ABORT_RECEIVE, 0);
  phuart->Initialize(SERIAL3_Event_Process);
  phuart->PowerControl(ARM_POWER_FULL);
  phuart->Control(ARM_USART_MODE_ASYNCHRONOUS |
                          ARM_USART_DATA_BITS_8 |
                          ARM_USART_PARITY_NONE |
                          ARM_USART_STOP_BITS_1 |
                          ARM_USART_FLOW_CONTROL_NONE,
                        pSerial_Device->BaudRate);
  phuart->Control(ARM_USART_CONTROL_TX, 1);
  phuart->Control(ARM_USART_CONTROL_RX, 1);
}
...
{
  SERIAL_PORT_HANDLE_Typedef_t *pSerial_Device = (SERIAL_PORT_HANDLE_Typedef_t *)pSerial_Port_Handle;
  const ARM_DRIVER_USART *phuart = (const ARM_DRIVER_USART *)pSerial_Device->pSerial_Handle;
...
static bool UART_Receive_Data_Start(void *pSerial_Port_Handle, uint8_t *Buffer, uint32_t Len, uint32_t BlockTime)
{
  ...
  /* 打开空闲中断，使能DMA接收 */
  if(0 != phuart->Receive(Buffer, Len))
  {
    return false;
  }
  return true;
...
static bool UART_Set_BaudRate(void *pSerial_Port_Handle, uint32_t BaudRate)
{
  ...
  switch(BaudRate)
  {
    ...
    case 4800:
    case 2400:
      if(pSerial_Device->BaudRate == BaudRate)
      {
        return false;
      }
      ...
      pSerial_Device->BaudRate = BaudRate;
...
static void UART_Receive_Half_IT(void *pSerial_Port_Handle)
{
  ...
  /* 计算已接收数据长度 */
  uint32_t Rec_Total = phuart->GetRxCount();
...
static void UART_Receive_Complete_IT(void *pSerial_Port_Handle)
{
  ...
  /* 数据回显 */
#if USE_LOOPBACK
  pSerial_Device->pSend_Data_Start(pSerial_Port_Handle, pSerial_Device->pReceive_Buffer + pSerial_Device->Last_Rec_Data_Size_Cnt, Rec_Len, 0);
#endif
...
static void UART_Idel_IT(void *pSerial_Port_Handle)
{
  ...
  /* 计算已接收数据长度 */
  uint32_t Rec_Total = pSerial_Device->Buffer_Size - phuart->GetRxCount();
 ...
 * @return false
 */
static bool CDC_Start(void *pSerial_Port_Handle)
{
 ...
 * @param huart 串口句柄
 */
void HAL_UART_RxHalfCpltCallback(void *huart)
{
...
void Serial_Port_IT_CallBack(void *Arg, uint32_t Size)
{
  for(int i = 0; i < SERIAL_PORT_NUM_MAX; i++)
  {
    ...
    {
      /* 检测到半完成中断类型 */
      if(Size == Serial_Device_List[i].Buffer_Size / 2U)
      {
        if(NULL != Serial_Device_List[i].pReceive_Half_IT)
        {
          Serial_Device_List[i].pReceive_Half_IT(&Serial_Device_List[i]);
...
{
  /* 初始化串口 */
  SERIAL_PORT_HANDLE_Typedef_t *pSerial_Device = &Serial_Device_List[SERIAL_PORT_UART_3];
  pSerial_Device->pSerial_Handle = (const void *)&Driver_USART3;

  // pSerial_Device->pCQ_Handle = CQ_Create(1024);

  CQ_init(&Serial_Port_Uart3_CQ, UART3_CQ_Buffer, sizeof(UART3_CQ_Buffer));
  pSerial_Device->pCQ_Handle = &Serial_Port_Uart3_CQ;

  pSerial_Device->pReceive_Buffer = UART3_DMA_Buf;//(uint8_t *)SERIAL_PORT_MALLOC(1024);
  pSerial_Device->Buffer_Size = sizeof(UART3_DMA_Buf);
  pSerial_Device->BaudRate = 115200U;
  ...
  /* 初始化 */
  pSerial_Device->pInit(pSerial_Device);

  if(NULL == pSerial_Device->pCQ_Handle)
  {
    printf("Serial Port Uart 3 Malloc Buf Faild.\r\n");
    return;
  }
  printf("Serial Port Uart 3 Init Ok.\r\n");
  ...
  pSerial_Device->pStart = CDC_Start;
